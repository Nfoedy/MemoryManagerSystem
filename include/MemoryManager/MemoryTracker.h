...
public:
    ...
    void Register(void* ptr, size_t size, const char* file, int line);   // Registra una nuova allocazione nella mappa

    // Rimuove un allocazione dalla mappa
    // Ritorna true se trovata, false se il puntatore non esiste
    bool Unregister(void* ptr, size_t& outSize);  

    void PrintLeaks() const;   // Stampa le statistiche base sull'utilizzo della memoria

    size_t GetActiveAllocations() const;   // Restituisce il numero di allocazioni attive
